...
imbalanced_train_dataset.train_data = np.delete(train_loader.dataset.data, idx_to_del, axis=0)
...
def train_meta(train_loader, validation_loader,model,vnet,optimizer_a,optimizer_c,epoch):
    ...
    for i, (inputs, targets, index) in enumerate(train_loader):
        ...
        cost_1 = F.cross_entropy(y_f_hat, target_var, reduce=False)
        cost_11 = torch.reshape(cost_1, (len(cost_1), 1))
        v_lambda = vnet(cost_11.data, target_var.data, c).squeeze(1)

        l_f_meta = (torch.sum( F.cross_entropy(y_f_hat, target_var, reduce=False) * v_lambda ))/(torch.sum(v_lambda).detach().data)
        ...
        l_f = (torch.sum( F.cross_entropy(y_f_hat, target_var, reduce=False) * v_lambda ))/(torch.sum(v_lambda).detach().data)
